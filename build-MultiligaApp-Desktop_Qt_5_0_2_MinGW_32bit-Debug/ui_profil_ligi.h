/********************************************************************************
** Form generated from reading UI file 'profil_ligi.ui'
**
** Created by: Qt User Interface Compiler version 5.0.2
**
** WARNING! All changes made in this file will be lost when recompiling UI file!
********************************************************************************/

#ifndef UI_PROFIL_LIGI_H
#define UI_PROFIL_LIGI_H

#include <QtCore/QVariant>
#include <QtWidgets/QAction>
#include <QtWidgets/QApplication>
#include <QtWidgets/QButtonGroup>
#include <QtWidgets/QCommandLinkButton>
#include <QtWidgets/QDialog>
#include <QtWidgets/QGroupBox>
#include <QtWidgets/QHeaderView>
#include <QtWidgets/QLabel>
#include <QtWidgets/QListView>
#include <QtWidgets/QPushButton>
#include <QtWidgets/QVBoxLayout>
#include <QtWidgets/QWidget>

QT_BEGIN_NAMESPACE

class Ui_profil_ligi
{
public:
    QGroupBox *groupBox_4;
    QListView *listView_3;
    QGroupBox *groupBox_3;
    QListView *listView_2;
    QGroupBox *groupBox_2;
    QListView *listView;
    QCommandLinkButton *commandLinkButton_2;
    QGroupBox *groupBox_5;
    QWidget *layoutWidget;
    QVBoxLayout *verticalLayout_2;
    QLabel *label_5;
    QLabel *label_6;
    QLabel *label_9;
    QLabel *label_10;
    QLabel *label_11;
    QGroupBox *groupBox;
    QWidget *layoutWidget_3;
    QVBoxLayout *verticalLayout;
    QLabel *label;
    QLabel *label_7;
    QLabel *label_3;
    QLabel *label_4;
    QLabel *label_8;
    QLabel *label_2;
    QCommandLinkButton *commandLinkButton;
    QPushButton *pushButton;
    QCommandLinkButton *commandLinkButton_4;

    void setupUi(QDialog *profil_ligi)
    {
        if (profil_ligi->objectName().isEmpty())
            profil_ligi->setObjectName(QStringLiteral("profil_ligi"));
        profil_ligi->resize(978, 715);
        groupBox_4 = new QGroupBox(profil_ligi);
        groupBox_4->setObjectName(QStringLiteral("groupBox_4"));
        groupBox_4->setGeometry(QRect(630, 10, 291, 411));
        listView_3 = new QListView(groupBox_4);
        listView_3->setObjectName(QStringLiteral("listView_3"));
        listView_3->setGeometry(QRect(10, 20, 271, 381));
        groupBox_3 = new QGroupBox(profil_ligi);
        groupBox_3->setObjectName(QStringLiteral("groupBox_3"));
        groupBox_3->setGeometry(QRect(320, 230, 291, 231));
        listView_2 = new QListView(groupBox_3);
        listView_2->setObjectName(QStringLiteral("listView_2"));
        listView_2->setGeometry(QRect(10, 20, 271, 201));
        groupBox_2 = new QGroupBox(profil_ligi);
        groupBox_2->setObjectName(QStringLiteral("groupBox_2"));
        groupBox_2->setGeometry(QRect(320, 10, 291, 211));
        listView = new QListView(groupBox_2);
        listView->setObjectName(QStringLiteral("listView"));
        listView->setGeometry(QRect(10, 20, 271, 181));
        commandLinkButton_2 = new QCommandLinkButton(profil_ligi);
        commandLinkButton_2->setObjectName(QStringLiteral("commandLinkButton_2"));
        commandLinkButton_2->setGeometry(QRect(10, 510, 231, 41));
        groupBox_5 = new QGroupBox(profil_ligi);
        groupBox_5->setObjectName(QStringLiteral("groupBox_5"));
        groupBox_5->setGeometry(QRect(10, 230, 299, 191));
        layoutWidget = new QWidget(groupBox_5);
        layoutWidget->setObjectName(QStringLiteral("layoutWidget"));
        layoutWidget->setGeometry(QRect(10, 20, 281, 161));
        verticalLayout_2 = new QVBoxLayout(layoutWidget);
        verticalLayout_2->setObjectName(QStringLiteral("verticalLayout_2"));
        verticalLayout_2->setContentsMargins(0, 0, 0, 0);
        label_5 = new QLabel(layoutWidget);
        label_5->setObjectName(QStringLiteral("label_5"));

        verticalLayout_2->addWidget(label_5);

        label_6 = new QLabel(layoutWidget);
        label_6->setObjectName(QStringLiteral("label_6"));

        verticalLayout_2->addWidget(label_6);

        label_9 = new QLabel(layoutWidget);
        label_9->setObjectName(QStringLiteral("label_9"));

        verticalLayout_2->addWidget(label_9);

        label_10 = new QLabel(layoutWidget);
        label_10->setObjectName(QStringLiteral("label_10"));

        verticalLayout_2->addWidget(label_10);

        label_11 = new QLabel(layoutWidget);
        label_11->setObjectName(QStringLiteral("label_11"));

        verticalLayout_2->addWidget(label_11);

        groupBox = new QGroupBox(profil_ligi);
        groupBox->setObjectName(QStringLiteral("groupBox"));
        groupBox->setGeometry(QRect(10, 10, 299, 211));
        layoutWidget_3 = new QWidget(groupBox);
        layoutWidget_3->setObjectName(QStringLiteral("layoutWidget_3"));
        layoutWidget_3->setGeometry(QRect(10, 20, 281, 181));
        verticalLayout = new QVBoxLayout(layoutWidget_3);
        verticalLayout->setObjectName(QStringLiteral("verticalLayout"));
        verticalLayout->setContentsMargins(0, 0, 0, 0);
        label = new QLabel(layoutWidget_3);
        label->setObjectName(QStringLiteral("label"));

        verticalLayout->addWidget(label);

        label_7 = new QLabel(layoutWidget_3);
        label_7->setObjectName(QStringLiteral("label_7"));

        verticalLayout->addWidget(label_7);

        label_3 = new QLabel(layoutWidget_3);
        label_3->setObjectName(QStringLiteral("label_3"));

        verticalLayout->addWidget(label_3);

        label_4 = new QLabel(layoutWidget_3);
        label_4->setObjectName(QStringLiteral("label_4"));

        verticalLayout->addWidget(label_4);

        label_8 = new QLabel(layoutWidget_3);
        label_8->setObjectName(QStringLiteral("label_8"));

        verticalLayout->addWidget(label_8);

        label_2 = new QLabel(layoutWidget_3);
        label_2->setObjectName(QStringLiteral("label_2"));

        verticalLayout->addWidget(label_2);

        commandLinkButton = new QCommandLinkButton(profil_ligi);
        commandLinkButton->setObjectName(QStringLiteral("commandLinkButton"));
        commandLinkButton->setGeometry(QRect(660, 420, 231, 41));
        pushButton = new QPushButton(profil_ligi);
        pushButton->setObjectName(QStringLiteral("pushButton"));
        pushButton->setGeometry(QRect(10, 430, 299, 28));
        commandLinkButton_4 = new QCommandLinkButton(profil_ligi);
        commandLinkButton_4->setObjectName(QStringLiteral("commandLinkButton_4"));
        commandLinkButton_4->setGeometry(QRect(10, 470, 231, 41));

        retranslateUi(profil_ligi);

        QMetaObject::connectSlotsByName(profil_ligi);
    } // setupUi

    void retranslateUi(QDialog *profil_ligi)
    {
        profil_ligi->setWindowTitle(QApplication::translate("profil_ligi", "Dialog", 0));
        groupBox_4->setTitle(QApplication::translate("profil_ligi", "Tabela:", 0));
        groupBox_3->setTitle(QApplication::translate("profil_ligi", "Nadchodz\304\205ce kolejki:", 0));
        groupBox_2->setTitle(QApplication::translate("profil_ligi", "Rozegrane kolejki:", 0));
        commandLinkButton_2->setText(QApplication::translate("profil_ligi", "Zmie\305\204 zasady ligi", 0));
        groupBox_5->setTitle(QApplication::translate("profil_ligi", "Domy\305\233lne zasady:", 0));
        label_5->setText(QApplication::translate("profil_ligi", "Ilo\305\233\304\207 kolejek:", 0));
        label_6->setText(QApplication::translate("profil_ligi", "Ilo\305\233\304\207 meczy na kolejk\304\231:", 0));
        label_9->setText(QApplication::translate("profil_ligi", "Liczebno\305\233\304\207 dru\305\274yn:", 0));
        label_10->setText(QApplication::translate("profil_ligi", "D\305\202ugo\305\233\304\207 meczu: np 2x45 min", 0));
        label_11->setText(QApplication::translate("profil_ligi", "Ilo\305\233\304\207 punkt\303\263w do zwyci\304\231stwa: brak / np 11", 0));
        groupBox->setTitle(QApplication::translate("profil_ligi", "Liga", 0));
        label->setText(QApplication::translate("profil_ligi", "Nazwa:", 0));
        label_7->setText(QApplication::translate("profil_ligi", "Dyscyplina: Dyscyplina X", 0));
        label_3->setText(QApplication::translate("profil_ligi", "Poziom ligi:", 0));
        label_4->setText(QApplication::translate("profil_ligi", "Sezon:", 0));
        label_8->setText(QApplication::translate("profil_ligi", "Miasto: Miasto X", 0));
        label_2->setText(QApplication::translate("profil_ligi", "Ogranizator: Opiekun X", 0));
        commandLinkButton->setText(QApplication::translate("profil_ligi", "Zobacz poprzedni sezon", 0));
        pushButton->setText(QApplication::translate("profil_ligi", "Powr\303\263t", 0));
        commandLinkButton_4->setText(QApplication::translate("profil_ligi", "Edytuj lig\304\231 ", 0));
    } // retranslateUi

};

namespace Ui {
    class profil_ligi: public Ui_profil_ligi {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_PROFIL_LIGI_H
